...
export default function App() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const { invoke } = await import("@tauri-apps/api/core");
        const r = await invoke<{ path: string; name: string; ext: string; isDir: boolean }[]>("search_files", { query: q, limit: 200 });
        setFsResults(r);
      } catch { setFsResults([]); }
    }, 150);
  ...
  const enhResults = useMemo<EnhResult[]>(() => {
    const tier2: EnhResult[] = fsResults.slice(0, 200).map(f => ({ kind: "fs" as const, path: f.path, name: f.name, ext: f.ext, isDir: f.isDir }));
  ...
  const changeSearchEngine = useCallback(async (eng: "builtin"|"everything") => {
    ...
    try {
      const { invoke } = await import("@tauri-apps/api/core");
      const r = await invoke<{ok:boolean;connected?:boolean}>("set_search_engine", { engine: eng });
      if (!r.ok) { setSearchEngine("builtin"); if (store) { await store.set("search-engine", "builtin"); await store.save(); } return; }
      setEverythingAvailable(r.connected ?? false);
    } catch {}
  }, [store]);
  // 设置页打开时取索引条数
              ...
              <div className="settings-panel">
                ...
                {settingsTab==="search" && (<>
                  ...
                  {searchEngine==="everything" && (
                    <p className={`settings-hint${everythingAvailable?"":" settings-hint-error"}`} style={{marginTop:6}}>
                      {everythingAvailable ? "● Everything SDK 已连接，全盘毫秒级搜索。" : "● Everything 未运行或未连接，查询已自动回退内置引擎。"}
                      {" "}<button className="settings-action" style={{fontSize:11,padding:"2px 8px",marginLeft:4}} onClick={async()=>{try{const{invoke}=await import("@tauri-apps/api/core");const r=await invoke<{ok:boolean;connected?:boolean}>("set_search_engine",{engine:"everything"});setEverythingAvailable(r.connected??false);}catch{}}}>重新检测</button>
                    </p>
                  )}
