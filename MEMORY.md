...
> **最后更新**：2026-07-01（内置引擎增强 ✅；Everything SDK DLL 集成 ✅——捆绑 DLL 零外部依赖）
>
> **关联文档**：规则铁律看 `CLAUDE.md`；决策根因看 `DECISIONS.md`；开发过程看 `DEVLOG.md`；本文件 = 项目现状快照 + 变更记录。
>
> **维护方式**：
...
- **修复（`src-tauri/src/lib.rs` `set_clipboard_image`）**：非桌面分支按目标窗口类再分叉——**文件夹窗口(`CabinetWClass`/`ExploreWClass`)→ CF_HDROP 落地真 PNG**：大图(有 `orig_path`)直接拿 `clip_images/{time}.png` 路径走 `write_cf_hdrop`（**零解码**，根除文件夹卡顿）;小图(无 orig_path)解一次 base64 写临时 PNG(temp dir，`workbench_clip_{time}.png`)，best-effort 后台延迟 5s 清理。**其余 app(Paint/聊天框)维持原 `set_image` 位图分支，行为不变**（含其大图解码，本次不动，避免一次多变量）。桌面分支 `desktop_copy_files` 不动。
...
- **关联**：DECISIONS §6 延伸「截图落地文件夹」;CLAUDE.md 剪贴板节新增三分叉不变式。
