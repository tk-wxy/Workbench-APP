...
- **新增（续45，纯前端清理 + UI 完善）**：**自定义热键 V2-2**——删 PROBE V2-0（CapRow/logCap/probingRef/capLog/probe CSS）和 V21-TEMP harness（v21TempCombo/segmented）。`changeHotkey` 签名放宽为 `string`，输入 normalize toLowercase；新增 `hotkeyInput` state（编辑态与已提交值分离，成功时同步）；store 加载放宽接受任意非空字符串。热键 tab 改文本输入框 + 应用按钮（Enter 触发）+ 格式提示行。底栏 `<kbd>` 改动态渲染（ctrl→Ctrl/shift→Shift/space→Space/方向键→箭头）。`.hotkey-input` CSS 替换 `.probe-*`。**tsc 零错误✓；GUI 待实测。**
