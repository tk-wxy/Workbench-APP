...
> **最后更新**：2026-07-02（续80：点击粘贴不稳定修复——焦点交还盲等 150ms 改守卫轮询）
>
> **关联文档**：agent 规则铁律统一看 `CLAUDE.md`（唯一入口，原 `AGENTS.md` 已于续80 移除）；决策根因看 `DECISIONS.md`；本文件 = 项目现状快照 + 变更记录。
>
> **维护方式**：
...
- **文档入口（续80 起单一化）**：`CLAUDE.md` 是唯一 agent 规则入口（续76 曾设 `AGENTS.md` Codex 副本，续80 应用户要求移除，杜绝双文档漂移）。PowerShell 读中文文档必须显式 `-Encoding utf8`。完成开发任务前继续强制更新本文件；§0 只放短快照，长细节放 §0A / 后续记录。
- **当前稳定功能**：Ctrl+Space 热键（长按 momentary + 短按 toggle，键态轮询驱动）+ Esc 关闭 + light dismiss；三类型剪贴板历史/粘贴/复制/持久化/图片原图缓存 janitor；中转区多选、批量 file、拖入/拖出；启动器收藏托盘、应用/文件启动、增强搜索与文件索引；设置面板（常规/启动台/中转站/剪贴板/搜索/快捷键/关于）。
- **最高危硬规则提醒**：窗口/焦点/热键/剪贴板改动前必须重读 `CLAUDE.md` 铁律；根因查 `DECISIONS.md`。尤其别改 `tauri.conf.json` 的 `transparent:true` / `focus:false`，别让前端管 hide，别把热键 show/hide 回退到 `RegisterHotKey` 事件，新增剪贴板读写必须过 `CLIPBOARD_LOCK`。
- **最近状态（续80）**：修「点击粘贴偶发失败但手动 Ctrl+V 能粘上」——根因是焦点交还流程的盲等 `sleep(150ms)` 竞态（hide 异步派发，负载高时前台未交接就注入 Ctrl+V）。`clipboard.rs` 新增 `wait_foreground_handback` 守卫轮询（前台非本窗口且非 NULL 即交接完成，超时 500ms 保底，落定余量 50ms，`FOCUS_HANDBACK_*` 常量），三处粘贴路径（paste/filepaste/imgpaste）统一替换并补齐带 tag 日志（文本路径原先零日志）。`cargo clippy` 8 条基线警告不变、零新增。**GUI 实测（2026-07-02，用户）：文本点击粘贴→Chrome 对话框成功**（日志 `[paste] handback waited=0ms timeout=false fg class="Chrome_WidgetWin_1"`，全程 114ms）；file/image 路径与高负载场景待更多日常使用观察。详见 §0A 与 DECISIONS §3 延伸（续80）。CLAUDE.md 铁律「焦点交还流程」已同步更新（sleep(150ms) 标注已废弃别回退）。**同会话应用户要求删除 `AGENTS.md`**（Codex 副本，与 CLAUDE.md 同源仅入口段措辞不同，删除零信息丢失），`CLAUDE.md` 成为唯一 agent 规则入口，全项目引用已清理。
- **上一状态（续79）**：设置面板按功能域重构：常规只保留全局外观/开机自启，启动台与中转站拆成独立条目，剪贴板只管历史与图片缓存，搜索页承接默认搜索模式与搜索引擎。新增启动台“添加应用/清空”管理入口；设置按钮 hover 区分普通动作与清空类危险动作。验证 `npx tsc --noEmit` + `npm run build` 通过；GUI 观感待用户实测。
- **下一步候选**：① 启动器键盘导航；② 文件结果右键「打开所在目录」/命中高亮回传；③ 索引目录可配置；④ 增强搜索纳入剪贴板条目；⑤ file/folder 收藏的非拖入入口。
- **阻塞 / 待决策**：无。

## 0A. 最近状态细节 〔保留详记，按需阅读〕

- **新增（续80，仅 `clipboard.rs` + 三份文档，GUI 粘贴链路未实测 2026-07-02）——点击粘贴不稳定修复（焦点交还守卫轮询）**：用户报「点击历史项粘贴偶发失败，但失败后手动 Ctrl+V 能粘上」。诊断结论：剪贴板写入在注入前已完成且不失败（所以手动能粘），失败全在「hide → 盲等 150ms → 注入 Ctrl+V」后半段——`window.hide()` 是异步派发（事件循环处理 + OS 激活交接都在返回后），负载高时 150ms 赌输，`GetForegroundWindow` 仍返回本窗口/NULL → Ctrl+V 注入进已隐藏的自家 WebView；还会污染 file/image 的 class 三分叉。修复（单变量）：新增 `wait_foreground_handback(&app, tag)`（10ms 采样至前台非本窗口且非 NULL，上限 500ms 超时保底继续，确认后 50ms 落定余量；常量 `FOCUS_HANDBACK_POLL_MS/MAX_MS/SETTLE_MS`；self hwnd 取法同 `start_focus_watch` 的 `h.0 as isize`），替换 `paste_clipboard`/`set_clipboard_files`/`set_clipboard_image` 三处 `sleep(150ms)`，日志带 tag（文本路径原先零日志、失败不可诊断，现补齐）。**已识别未修**（详见 DECISIONS §3 延伸续80）：① show 时未快照原前台 HWND（`SetForegroundWindow(GetForegroundWindow())` 恒等空操作）——结构性改流程暂缓；② UIPI 提权目标静默吞 SendInput——无解只能将来提示；③ 物理修饰键未中和。**验证**：`cargo clippy` 通过，8 条基线警告不变、新代码零警告；真实点击粘贴链路属 GUI 无头不可驱动，交付时未实测；**用户实测（2026-07-02）文本路径→Chrome 对话框成功**（waited=0ms / timeout=false / class 正确，全程 114ms，比旧盲等还快）。若日后仍偶发失败，先看 `[paste]/[filepaste]/[imgpaste] handback` 日志的 timeout 与 fg class 定位是 ①还是②。文档同步：CLAUDE.md 铁律焦点交还流程改为守卫轮询并标注旧盲等已废弃；DECISIONS §3 延伸（续80）记完整根因。文件：`src-tauri/src/clipboard.rs`。（注：同会话稍后 `AGENTS.md` 已应用户要求删除，见 §0。）
- **新增（续79，纯前端，GUI 未实测 2026-07-02）——设置内容按功能域重构**：`SETTINGS_TABS` 从常规/剪贴板/搜索/快捷键/关于扩为 **常规 / 启动台 / 中转站 / 剪贴板 / 搜索 / 快捷键 / 关于**。常规页只保留背景主题与开机自启；启动台页显示收藏数量、提供添加应用与清空入口，并标注手动排序；中转站页承接显示布局与清空中转条目；剪贴板页移除中转站内容，仅保留历史条数、清空历史、图片原图缓存；搜索页承接呼出默认搜索模式与搜索引擎/额外目录配置。CSS 调整 `.settings-action`：普通按钮 hover 为中性样式，清空类按钮加 `.danger` 才用红色 hover；新增 `.settings-inline-actions` / `.settings-row-value`。**验证**：`npx tsc --noEmit` 通过，`npm run build` 通过。文件：`src/App.tsx` / `src/App.css`。
...
- **待做（续75 实测反馈，已归档待实现）**：⓪a **启动台拖拽舍去抓手光标**——grab/grabbing 实测卡顿，回退光标改动（`.app-tile` cursor 恢复、`.launcher-reordering` 去 grabbing）；⓪b **启动台拖拽被拖项目跟随观感**——源 `opacity:0` 后拖动时项目"消失"，用户要项目明确跟在光标上；先加日志确认 ghost 是否真的跟手到位，再决定强化跟随/让源半可见。
- **下一步（候选，无阻塞）**：① 启动器键盘导航（←→↑↓ + Enter）；② 文件结果右键「打开所在目录」(`reveal_in_explorer`) + 高亮区间回传（Rust 回传命中 ranges）；③ 索引目录可配置（设置面板扩项）；④ 增强搜索 Tier 2 剩余（剪贴板条目纳入）；⑤ file/folder 收藏的非拖入入口（如文件选择对话框）；⑥ ~~拖出（drag-out）~~ **已实现 + GUI 实测通过（续71/71b，2026-06-29）**——核心路径（file/folder 拖到异地、MOVE 移除、不卡死、不出 download.png）已验证；剩 text→记事本、image→Paint(CF_DIB)/Explorer(CF_HDROP)、Esc 取消、与拖入并发 这几条顺手补测即可（功能在、低风险）；⑦ T9 渲染进程重建后拖入失效（已知罕见限制，低优先级）
