...
> **最后更新**：2026-07-01（续78：启动台拖拽预览改为专用 drag-layer 承载）
>
...
- **最近状态（续78）**：启动台拖拽预览已从“直接挂 `document.body` 并给 ghost 抢 z-index”收敛为专用顶层 `.drag-layer` 承载 DOM clone；ghost 在 layer 内用 absolute 定位，层级集中到容器 `z-index:100003`，保留 body fallback。排序算法、让路、松手回落、视觉参数不变。静态验证 `npx tsc --noEmit` + `npm run build` 通过；GUI 手感待用户实测。
