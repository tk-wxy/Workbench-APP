...
- **最近状态（续80）**：修「点击粘贴偶发失败但手动 Ctrl+V 能粘上」——根因是焦点交还流程的盲等 `sleep(150ms)` 竞态（hide 异步派发，负载高时前台未交接就注入 Ctrl+V）。`clipboard.rs` 新增 `wait_foreground_handback` 守卫轮询（前台非本窗口且非 NULL 即交接完成，超时 500ms 保底，落定余量 50ms，`FOCUS_HANDBACK_*` 常量），三处粘贴路径（paste/filepaste/imgpaste）统一替换并补齐带 tag 日志（文本路径原先零日志）。`cargo clippy` 8 条基线警告不变、零新增。**GUI 粘贴链路未实测**，待用户真机验证；详见 §0A 与 DECISIONS §3 延伸（续80）。CLAUDE.md 铁律「焦点交还流程」已同步更新（sleep(150ms) 标注已废弃别回退）。**同会话应用户要求删除 `AGENTS.md`**（Codex 副本，与 CLAUDE.md 同源仅入口段措辞不同，删除零信息丢失），`CLAUDE.md` 成为唯一 agent 规则入口，全项目引用已清理。
