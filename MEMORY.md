...
> **最后更新**：2026-07-02（续79：设置内容按功能域重构）
>
...
- **最近状态（续79）**：设置面板按功能域重构：常规只保留全局外观/开机自启，启动台与中转站拆成独立条目，剪贴板只管历史与图片缓存，搜索页承接默认搜索模式与搜索引擎。新增启动台“添加应用/清空”管理入口；设置按钮 hover 区分普通动作与清空类危险动作。验证 `npx tsc --noEmit` + `npm run build` 通过；GUI 观感待用户实测。
