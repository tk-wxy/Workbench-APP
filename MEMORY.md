...
> **最后更新**：2026-06-28（续58：搜索框上移+目录路径；续59：悬浮操作按钮；续60：操作按钮 ✓ 反馈；续61：中转区双布局；续62：方格卡片对齐 app-tile；续63：中转区文件/文件夹真实 Windows 图标；续64：呼出默认搜索设置）
>
...
- **新增（续64，纯前端，零 Rust 改动）——呼出默认搜索设置（GUI 实测通过）**：设置 → 常规 新增「呼出默认搜索」seg（界面搜索/增强搜索），持久化 store key `search-default-mode`。**核心行为**：enhanced 模式下顶栏 onChange 同步写 `search`+`enhQuery`，**不移动焦点**（彻底避免 IME 被打断），`v` 非空时 `setEnhOpen(true)`，空时 `setEnhOpen(false)` 回界面。**IME 兼容**：不依赖 `isComposing`（WebView2 环境不可靠），焦点留在顶栏，IME 组字自然在顶栏完成，组字结果同步到 enhQuery。**Ctrl+K 行为**：enhanced 模式下 Ctrl+K/Esc 关闭 enh-layer 时设 `pageSearchForcedRef=true`（本次呼出期间打字走界面搜索），再按 Ctrl+K 重开 enh-layer 时清 flag；`hotkey-hide` 重置 flag。**清空关闭**：顶栏清空或 enhInput 清空（用户点进 enh-layer 后退格到空）均触发 `setEnhOpen(false)` 回界面、焦点还顶栏。底栏提示：`enhOpen ? "界面搜索" : "增强搜索"`。enh-layer hint：enhanced 模式显示「Ctrl+K 界面搜索 · Esc 关闭」。文件：`src/App.tsx`（+searchDefaultMode state/ref/callback/store load，+pageSearchForcedRef，顶栏 onChange 重写，enhInput onChange 加清空关闭，Ctrl+K/Esc/hotkey-hide 加 flag 管理）。
