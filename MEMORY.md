...
> **最后更新**：2026-07-01（续77：启动台拖拽更换位置细节打磨——悬浮副本跟手 + 去抓手光标）
>
...
- **最近状态（续77）**：启动台拖拽更换位置继续打磨：去掉 `grab/grabbing` 抓手光标，拖拽期间只禁选中文本；ghost 改为直接 `cloneNode` 源卡片并插入 `document.body` 的悬浮副本，保持鼠标按下时在卡片内的抓取偏移，像系统桌面拖图标一样跟手。续77b 修正真实不可见根因：`#overlay z-index:99999` 压住了 `body` 下 `z-index:9999` 的 ghost，现提到 `100003` 并加内联兜底；续77c 按用户反馈把悬浮副本调淡并去阴影。排序算法、让路与松手回落逻辑未动。静态验证 `npx tsc --noEmit` + `npm run build` 通过；GUI 手感待用户实测。
